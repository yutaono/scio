...
object BlockingBigtableRead {
  def main(cmdlineArgs: Array[String]): Unit = {
    ...
    sc.parallelize(letters)
      .applyTransform(ParDo.of(new FillDoFn(1000000)))
      .applyTransform(ParDo.of(new BlockingBigtableDoFn[String, String](bigtableOptions) {
        override def processElement(session: BigtableSession, input: String) = {
          val s = input
          val key = ByteString.copyFromUtf8(s"key-$s")
          ...
          val request = readRowsRequestBuilder
            ...
            .build()
          val result = session.getDataClient.readFlatRowsList(request)
